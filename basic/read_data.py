...
class DataSet(object):
    ...
    def set_data(self, data): # for demo purpose only
        self.data = data
...
def read_data_demo(config, data_type, ref, data=None, data_filter=None, data_set=None):
    if data is None:
        data_path = os.path.join(config.data_dir, "data_{}.json".format(data_type))
        with open(data_path, 'r') as fh:
            data = json.load(fh)
    if data_set is None:
        ...
        # print("Loaded {}/{} examples from {}".format(len(valid_idxs), num_examples, data_type))
        shared = read_shared_data_demo(config, data_type, ref, data_filter)
        data_set = DataSet(data, data_type, shared=shared, valid_idxs=valid_idxs)
    else:
...
def read_shared_data_demo(config, data_type, ref, data_filter=None):
